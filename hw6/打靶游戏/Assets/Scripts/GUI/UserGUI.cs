...
public class UserGUI : MonoBehaviour {
    ...
    private void OnGUI() {
        if(game_start) {
            ...
            GUI.Label(new Rect(Screen.width - 120, 30, 200, 50), action.GetWind(), text_style);
        }
        else {
            GUI.Label(new Rect(Screen.width / 2 - 40, Screen.width / 2 - 320, 100, 100), "简单打靶", text_style);
